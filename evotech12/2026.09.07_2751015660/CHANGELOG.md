...
### Added
 - evoNet channel discovery (CAN Bus > "Enable evoNet channel discovery", off by default): the ECU now describes its own channels over CAN -- name, unit, scaling, display range and whether the vehicle actually has that sensor configured -- so a display can list and read them without having the layout hardcoded in its own firmware. Adding a sensor in the tune makes its channel show up on the display within about a second, with no display update. Costs 2 CAN frames per second when enabled; the classic Evotech CAN broadcast is unchanged and keeps working alongside it
 - evoNet fault reporting over CAN: a display can now show why the ECU is limiting the engine, not just that something is wrong -- severity, active fault count, the worst active code, the limp reason, and a freeze frame (RPM, coolant, throttle, battery) captured when each fault first appeared
 - evoNet remote actions over CAN: trip reset, launch control, anti-lag, valet mode and engine start/stop can be driven from a display. High-risk actions (start, valet) are behind their own separate switch ("Allow evoNet high-risk actions", off by default, independent of channel discovery), need a challenge-response exchange, and must pass the ECU's own preconditions -- engine stopped, no active cut, vehicle stopped, each of which refuses when it cannot be checked. A single stray CAN frame can never act on its own, and anything the display holds is released within 200ms if it stops talking
 - FatFS update to R0.16
 - TunerStudio dashboard: warning indicator now shows the last warning's error code instead of a blank "Warning" label
 - Idle Settings: exposed "IAC Output Filter Time Constant" (was already active in firmware, but not tunable/visible in TunerStudio before this)
 - Torque Smoothing (Performance > Torque Smoothing): optional ignition-based smoothing for everyday torque transitions (fuel-cut/WOT, WOT/coasting) plus an opt-in clutch-release overspeed guard and an opt-in advance-ceiling torque add. Disabled by default -- engine behavior is unchanged until explicitly turned on. Only tunable/visible on dev-branch builds (no bench/field validation yet) -- prod builds always run it at its safe default, see "dev-only" note below.
 - Anti-Lag ignition retard table (Performance > Anti-Lag > Ignition adjustment) now allows down to -90 deg, up from -60 deg

### Fixed
 - OBD-II responder: fixed several conformance bugs that caused slow or partial connections with wideband displays/scanners over OBD-II (e.g. WideLink) -- unsupported PIDs now get a proper negative response instead of silence (discovery time drops from ~25 seconds to under 1 second), lambda/battery voltage/ethanol%/oil temp/MAF are now correctly announced as supported (they were already being answered, just never advertised), Engine Load (PID 0x04) no longer incorrectly reports 100% on speed-density tunes, and both an alternate 11-bit addressing mode and 29-bit addressing are now accepted in addition to the original 11-bit mode. Reading and clearing OBD-II trouble codes (modes 03/07/0A/04) now actually works (was a no-op before)
 - Watchdog reset during boot: the boot sequence had no watchdog budget of its own -- it inherited the tight 500ms runtime timeout with nothing feeding it until the engine's periodic loop actually started, well after config was loaded from flash and hardware/evoNet init ran. Boot now gets a generous 7-second budget (same one already used for slow storage init), restored to the normal 500ms once the engine loop takes over
 - Flash journal tail-usage telemetry (visible to the tuning manager, not a TunerStudio-facing setting) was read off its dedicated storage thread, from whichever thread happens to poll live data -- with a tuning tool connected across a reset and the journal fairly full, this could show up as a burst of internal warnings right at boot and is a likely contributor to the watchdog reset above. Now cached and refreshed only from the storage thread itself
 - evoNet: a display that committed a channel subscription with zero channels selected got an active-but-empty subscription, which silently replaced the always-on default profile and left the display with no data streaming at all. An empty commit now falls back to the default profile instead
 - WideLink command frame (valet mode, launch control and anti-lag CAN overrides) no longer latches when the display stops talking: if the frame goes missing for 500ms the ECU releases all three back to what the tune says, instead of staying stuck until the next reboot. Previously a WideLink that crashed or got unplugged while valet mode was on left the engine RPM-limited with no way to recover short of power-cycling the ECU
 - Firmware update recovery: if the ECU crashes right after a firmware update (not just a full lock-up, but also a caught internal fault/assertion), it now falls back to bootloader/DFU mode automatically after 5 failed boot attempts (previously this specific case -- and the limit was 10 attempts, not 5 -- could loop indefinitely without ever falling back, requiring an external reflash tool to recover). Additionally, if the bootloader stays resident and nobody claims it within 5 seconds, it now drops into the ST DFU bootloader on its own, so a generic USB DFU flashing tool can recover the device without needing a bootloader-aware client
 - Closed-loop fuel trim (STFT): fixed oscillation that could show up at idle, previously only fixable by manually raising the base correction time constant (which also slowed down correction everywhere else, including WOT). The fast-trim error now uses the already-filtered lambda reading instead of the raw sensor, the output smoothing filter no longer eats into the stability margin the correction speed is derived from, and the internal exhaust temperature estimate no longer underestimates delay at idle.
 - live data output channels (odometer/fuel economy, idle state, ETB telemetry, etc.) could randomly go missing from a build's TunerStudio project depending on incidental build-cache state, even with no source changes -- this was a non-deterministic codegen bug, now fixed
 - TunerStudio dashboard: merged redundant clutch, idle/decel, and TPS1/TPS2 indicators into single indicators that show which side is active; clarified the "Thermal OK" label to say it's the fan's CLT-triggered failsafe; "Tuning" indicator no longer shows a blank label when idle
 - `dfcoActive` telemetry channel was stuck permanently false during a fuel cut (compared `cutFuel()` against a threshold it can never go below in steady-state) -- now reads DFCO's own state directly
 - Anti-Lag ignition retard (Performance > Anti-Lag) was silently dropped whenever a separate idle timing table was in use ("Use separate ignition table for idle") and the engine was idling with the throttle closed -- the exact condition Anti-Lag normally runs in. The engine ran the raw idle-table timing point instead of the configured Anti-Lag retard. Anti-Lag's ignition retard now always applies on top of whichever timing table (idle or main) is active
 - Burn no longer causes a small output bump on ETB/VVT/boost/idle/alternator when only a PID gain was changed (previously reset the I/D accumulator every time, even for a pure gain tweak) -- also fixed a case where the trigger decoder's own reconfiguration was never gated to RPM=0 at all; now retries safely on the next Burn once the engine actually stops
 - evotech12plus's TunerStudio project had an empty pin naming scheme for crank-based trigger types (would have broken crank sync detection) and a stale, smaller config layout than the source it should have been built from -- regenerated correctly from its real board definition
 - Torque Smoothing and Virtual Flex Sensor settings are no longer reachable on production builds even if a dev build's saved tune had them turned on -- the ECU now resets both back to their safe defaults on every boot and every Burn on a prod build, and their TunerStudio dialogs are hidden there too

### Breaking Changes
 - DFCO's own ignition retard fields (`dfcoRetardDeg`, `dfcoRetardRampInTime`) were removed -- that smoothing now lives in the new Torque Smoothing module's fields. Existing tunes are not automatically migrated; recalibrate DFCO entry smoothing under Performance > Torque Smoothing if you relied on the old fields.

### Removed
 - TunerStudio dashboard: removed the WBO2 fault indicator and the ETB RPM limit indicator (already covered by the Injection/Ignition OK indicators)
